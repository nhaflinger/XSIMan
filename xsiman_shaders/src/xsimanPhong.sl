...
)
{
	...
	if (transparencymap != "") {
		transparencycolor *= refract_inuse * texture(transparencymap, ss, tt, "blur", transparencymapblur);
	}
	else {
		transparencycolor = refract_inuse * transparency;
	}
	if (inverttrans != 0) {
	...
	if (shinymap != "") {
		roughness *= texture(shinymap, ss, tt, "blur", shinymapblur);
	}
