...
)
{
	...
	if (roughnessmap != "") {
		roughval *= texture(roughnessmap, ss, tt, "blur", roughnessmapblur);
	}
	...
	color spec = phong(Nf, V, roughval);
	specularcolor *= spec;
	...
	ambientcolor *= ambient() + ambienceval;
		...
		specularcolor * specular(Nf, V, roughval) + incand_inuse * inc_inten * incandescence;
