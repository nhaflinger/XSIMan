...
void getmacaddr_win(unsigned char macaddr[])
{
	int trace = 0;
    ...
    if (m_hInst < (HINSTANCE) HINSTANCE_ERROR) {
        m_hInst = NULL;
		fprintf(stderr, "failed loading inetmib1.dll\n");
        return;
    }
    m_Init =
    ...
    ret =
        m_Query(ASN_RFC1157_GETNEXTREQUEST, &varBindList, &errorStatus,
                &errorIndex);
    if (trace > 0) printf("# of adapters in this system : %i\n",
    ...
    do {
        ...
        ret =
            ...
            if (trace > 0) printf("Interface #%i type : %i\n", j, dtmp);
            ...
            if (dtmp == 6) {
                ...
                if ((!ret)
                    ...
                    if (
                        ...
                        if (trace > 0) printf("Interface #%i is a DUN adapter\n", j);
                        continue;
                    }
                    if (
                        (varBind[1].value.asnValue.address.stream[0] ==
                        ...
                        && (varBind[1].value.asnValue.address.stream[5] ==
                            0x00)) {
								...
								interfaces */
								if (trace > 0) printf("Interface #%i is a NULL address\n", j);
								continue;
							}
							if (trace > 0) {
					...
					for( int k=0; k < 6; k++ ) {
						macaddr[k] = varBind[1].value.asnValue.address.stream[k];
					}
            } //dtmp==6
        } //if !ret
