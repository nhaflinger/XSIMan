...
CStatus XSILoadPlugin( XSI::PluginRegistrar& in_reg )
{
	...
	in_reg.PutName( L"sdl2spdl" );
	in_reg.PutVersion( 1, 0 );

	// register a command plugin item.
	in_reg.RegisterCommand( L"sdl2spdl", L"sdl2spdl" );
...
extern "C"
#endif
XSI::CStatus sdl2spdl_Execute( XSI::CRef& in_context )
{
	Application app;
...
CStatus GetUserSelectedShader(CString& pathName, CString& baseName)
{
	...
	filebrowser.PutProperty(L"Filter", CValue(L"3Delight Shader (*.sdl)|*.sdl||"));
	CValue index; CValueArray args;
