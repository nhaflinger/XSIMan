...
#ifdef PRMAN 
	if (fileExt != CString(L"slo")) { 
		app.LogMessage(L"This is not a PRMan shader!");
		return CStatus::Fail;
	}
		...
		if (arg->svd_valisvalid) {
			...
			switch (arg->svd_type) {
				...
				case SLO_TYPE_MATRIX: {
					...
					char* svd_spacename = arg->svd_spacename;
...
void xsiSloInfo::write(FILE* stream, CRef& in_ref)
{
	...
	if (getArgType(shaderType) == L"material" || getArgType(shaderType) == L"light" || getArgType(shaderType) == L"volume" || 
		getArgType(shaderType) == L"imager") {
	...
	fwprintf(stream, L"\tRenderer \"prman\"\n");
	fwprintf(stream, L"\t{\n");
	fwprintf(stream, CString(L"\t\tName = \"" + shaderName + L"\";\n").GetWideString());
