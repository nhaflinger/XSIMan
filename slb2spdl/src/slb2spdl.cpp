...
CStatus GetUserSelectedShader(CString&, CString&, CString&);
...
CStatus XSILoadPlugin( XSI::PluginRegistrar& in_reg )
{
	...
	in_reg.PutName( L"slb2spdl" );
	in_reg.PutVersion( 1, 0 );

	// register a command plugin item.
	in_reg.RegisterCommand( L"slb2spdl", L"slb2spdl" );
...
extern "C"
#endif
XSI::CStatus slb2spdl_Execute( XSI::CRef& in_context )
{
	Application app;
	...
	CString shaderFilePath, shaderBaseName, filePath;
	st = GetUserSelectedShader(shaderFilePath, shaderBaseName, filePath);
...
CStatus GetUserSelectedShader(CString& pathName, CString& baseName, CString& filePath)
{
	...
	filebrowser.PutProperty(L"Filter", CValue(L"Air Shader (*.slb)|*.slb||"));
	CValue index; CValueArray args;
